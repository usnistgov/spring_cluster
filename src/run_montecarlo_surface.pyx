...
from montecarlo_cluster3_parallel_surface import montecarlo_cluster_surface
...
def run_montecarlo_surface(phiobj,starting_energy, use_atom_strain_cluster, beta, chem_pot, nsteps_arr, step_size_arr, report_freq, A, np.ndarray[DTYPE_t, ndim=2] coords, types, list dims, list phi_tensors, list nonzeros, cell = [], runaway_energy=-20.0, correspond=None, startonly=False, neb_mode=False, surface=[]):
...
#structures are stored for later analysis.
  ...
  #SURFACE
  print phiobj.supercell, 'TRUNCATE SURFACE', surface
  for s in range(ncells):
    sind = phiobj.index_supercell_f(s)
    if sind[2] in surface:
      for at in range(nat):
        UTYPES[(at) * ncells + s,:] = 0.0
    for at in range(nat):
      print at, sind, UTYPES[(at) * ncells + s,:]
  print '-------------'
  print
  surface_num = min(surface)
  print 'surface_num',surface_num
  print 
...
#    if phiobj.parallel:
    denergy, UTYPES = montecarlo_cluster_surface(interaction_mat, interaction_len_mat, cluster_sites,  supercell_add,  atoms_nz, strain, coords_unitcells, coords_ref, phiobj.Acell_super, nonzero_huge_hugeT,      phi_huge_huge, UTYPES, supercell, surface_num ,phiobj.magnetic,phiobj.vacancy,nstep, seed, beta, chem_pot,magnetic_aniso, dim_max,cluster_sites.shape[0], ncells, nat, nonzero_huge_hugeT.shape[1],      nonzero_huge_hugeT.shape[0],      supercell_add.shape[0],supercell_add.shape[1],interaction_mat.shape[0], dim_u )
#    else:
...
#in this case, where we only do step size determination, we return final structure, usually for recursive model improvement
  if nsteps_arr[1] == 0 and nsteps_arr[2] == 0 and nsteps_arr[0] > 0:
    ...
    if phiobj.magnetic == 0 or phiobj.magnetic == 1 :
      outstr,A, pos, types = output_struct(phiobj, ncells, struct_all[:,:,:,tocalculate], strain_all[:,:,tocalculate], cluster_all[:,:,tocalculate])
    elif phiobj.magnetic == 2: #heisenberg case    
      outstr,A, pos, types = output_struct(phiobj, ncells, struct_all[:,:,:,tocalculate], strain_all[:,:,tocalculate], cluster_all[:,:,tocalculate], output_magnetic=False)

    if phiobj.magnetic == 0 or phiobj.magnetic == 1 :
      outstr2,A7, pos7, types7 = output_struct(phiobj, ncells, (struct_all[:,:,:,tocalculate]-coords_ref)*0.7+coords_ref , 0.5*strain_all[:,:,tocalculate], cluster_all[:,:,tocalculate])
    elif phiobj.magnetic == 2: #heisenberg case    
      outstr2,A7, pos7, types7 = output_struct(phiobj, ncells, (struct_all[:,:,:,tocalculate]-coords_ref)*0.7+coords_ref , 0.5*strain_all[:,:,tocalculate], cluster_all[:,:,tocalculate], output_magnetic=False)
