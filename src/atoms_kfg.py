...
#            print cell
    ...
    def compare_pos(self,p1,p2):

        for i in range(3):
            if abs(p1[i] - p2[i]) > 1e-7 and abs(abs(p1[i] - p2[i])-1.0) > 1e-7:
                return False

        return True
#        elif abs(p1[0] - p2[0])-1 < 1e-7 and abs(p1[1] - p2[1])-1 < 1e-7 and abs(p1[2] - p2[2])-1 < 1e-7:
#            return True
    def find_atom(self,pos):
        for i,p in enumerate(self.scaled_positions):
#            print p
            if self.compare_pos(p,pos):
                return i
        return -1

    def find_atoms(self,pos):
        ret = []
        for p in pos:
            ret.append(self.find_atom(p))
        return ret
    def printer(self):
        print '++++++++'
        print 'cell'
...
atom_data = [ 
    ...
    [ 25, "Mn", "Manganese", 51.9961], # 25      #    [ 25, "Mn", "Manganese", 54.938045], # 25
